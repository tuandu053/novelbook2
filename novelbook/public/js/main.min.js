...
let speechSynthesisInstance; // Biến lưu trữ instance hiện tại
...
$('body').on('click', '.ntp_get_content', function () {
    const tempDiv = $('.ntp_content').html();
    const plainText = $(tempDiv).text();

    playSpeech(plainText);
});

$('body').on('click', '.ntp_pause', function () {
    pauseSpeech();
});

$('body').on('click', '.ntp_resume', function () {
    resumeSpeech();
});

$('body').on('click', '.ntp_stop', function () {
    stopSpeech();
});
  ...
  function playSpeech(content) {
    if ('speechSynthesis' in window) {
        // Dừng bài đọc hiện tại trước khi phát mới
        if (speechSynthesisInstance) {
            window.speechSynthesis.cancel();
        }

        // Tạo đối tượng đọc
        speechSynthesisInstance = new SpeechSynthesisUtterance(content);
        speechSynthesisInstance.lang = 'vi-VN'; // Đặt ngôn ngữ là tiếng Việt

        // Khi kết thúc bài đọc
        ...
        isPaused = false; // Reset trạng thái
    } else {
...
function resumeSpeech() {
    if ('speechSynthesis' in window) {
        if (isPaused) {
            ...
            isPaused = false; // Đặt lại trạng thái
        }
    } else {
        alert("Trình duyệt của bạn không hỗ trợ chức năng đọc thành tiếng!");
    }
